...
import { addItem } from 'features/Cart/model/slice/cartSlice';
import { useModalContext } from 'shared/context/ModalContext';
import { addItemToServer } from 'features/Cart/model/services/cartThunks';
        ...
        <section className={styles.pricing}>
            ...
            <div className={styles.cards}>
                {items.map((item) => {
                    ...
                    const displayPrice = selected === 'single' ? item.price ?? item.original : item.coursePrice ?? item.original;
                    ...
                    return (
                        <div className={styles.card} key={uniqueKey}>
                            ...
                            <select
                                value={selected}
                                onChange={(e) => handleSelectChange(item.name, e.target.value as 'single' | 'course')}
                            >
...
<button
  ...
  onClick={() => {
    const quantity = selected === 'course' ? 6 : 1;
    const price = selected === 'single' ? item.price : item.coursePrice;
    const cartItem = {
      ...
      duration: item.original,
      tag: activeTab,
      itemTag: 'LHR',
    };

    dispatch(addItem(cartItem));
    dispatch(addItemToServer(cartItem));
    toggleModal('cart');
  }}
>
                            <span onClick={() => toggleModal('cart')}>დაჯავშნა</span>
